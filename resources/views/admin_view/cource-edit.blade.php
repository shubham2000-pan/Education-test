  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Cource Form Update</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Cource Form Update</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
        ...
        <div class="col-md-12">
            <div class="card card-success">
              ...
              <div class="card-header">
                <h3 class="card-title">Update Cource</h3>
              </div>
                <div class="card-body">
                  <form action="{{ url('cource-update') }}" method="post">
                   @csrf
                   <input type="hidden" name="id" value="{{$result->id}}">
                   <label for="exampleInputEmail1">Name</label>
                <input class="form-control" type="text" name="name" placeholder="Name" value="{{ $result->name }}">
                <span class="error">{{ $errors->errors->first('name') }}</span> 
              
                <br>
                <label for="exampleInputEmail1">Description</label>
                <textarea id="summernote" name="description" placeholder="Description" value="" >
                 {{ $result->description }}
                  </textarea>
                ...
                <input class="form-control" type="text" name="duration" placeholder="Duration" value="{{ $result->duration }}" >
                <span class="error">{{ $errors->errors->first("duration") }}</span> 
              
                <br>
                <label for="exampleInputEmail1">Fees</label>
                <input class="form-control" type="text" name="fees" placeholder="fees" value="{{ $result->fees }}" >
                ...
                <option value="{{$key }}" {{ $key == $result->added_by ? 'selected=""' : '' }}>{{ $value}}</option>
                 @endforeach
                </select>
                ...
                <option value="{{$key }}" {{ $key == $result->fees_type ? 'selected=""' : '' }}>{{ $value}}</option>
                @endforeach
                </select>
